...
# Pairwise correlation of log2 fold changes for shared DEGs (padj < 0.05)
...
pairwise_plot <- ggplot(common_genes, aes(x = log2FC_OSNs, y = log2FC_Fatbody)) +
  ...
  labs(title = "Pairwise Expression of Common Genes (padj < 0.05)",
       x = "log2 Fold Change (OSNs)", y = "log2 Fold Change (Fatbody)")

ggsave("Pairwise_Correlation_CommonGenes.pdf", pairwise_plot, width = 7.5, height = 7.5, dpi = 300)
